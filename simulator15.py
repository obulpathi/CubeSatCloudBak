...
from cloud.transport.worker15 import *
...
class WorkerThread(threading.Thread):
    def __init__(self, mconfig, gsconfig, worker):
        ...
        self.fromWorkerToCSServers = []
        for i in range((NSIZE-5)/5):
            self.fromWorkerToCSServers.append(Queue.Queue())
        self.fromCSServersToWorker = []
        for i in range((NSIZE-5)/5):
            self.fromCSServersToWorker.append(Queue.Queue())
        threading.Thread.__init__(self)
    ...
    def run(self):
        reactor.connectTCP(self.mconfig.address, self.mconfig.port,
                            TransportWorkerFactory(self.worker.address, self.worker.homedir,
                                                    ...
                                                    self.fromWorkerToCSServers, self.fromCSServersToWorker))
        ...
        reactor.listenTCP(self.worker.port, 
                            TransportCSServerFactory(self.fromWorkerToCSServers, self.fromCSServersToWorker))
...
if __name__ == "__main__":
    ...
    for i in range(5, 15):
        ...
        wconfig.address = i
        wconfig.port = wconfig.port + i
        mconfig = copy.deepcopy(config.master)
        mconfig.port = wconfig.port - 5
        gsconfig = copy.deepcopy(config.groundstation)
        gsconfig.port = gsconfig.port + 5
        worker = WorkerThread(mconfig, gsconfig, wconfig)
        worker.start()
        workers.append(worker)
    sleep(1)
    # start the reactor
