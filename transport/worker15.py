...
class TransportWorkerProtocol(LineReceiver):
    def __init__(self, factory, homedir):
        ...
        self.cswaiter0 = WaitForData(self.factory.fromCSServersToWorker[0], self.getData0)
        self.cswaiter1 = WaitForData(self.factory.fromCSServersToWorker[1], self.getData1)
        self.ccwaiter = WaitForData(self.factory.fromCSClientToWorker, self.getReply)
        self.cswaiter0.start()
        self.cswaiter1.start()
        self.ccwaiter.start()
        self.mutexpr = Lock()
        self.mutexsp = Lock()
        self.fragments = None
    ...
    def getData0(self, data):
        fields = data.split(":")
        worker = fields[1]
        if worker not in self.rt:
            print "&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&"
            self.rt[worker] = self.factory.fromWorkerToCSServers[0]
        self.sendLine(data)
    ...
    def getData1(self, data):
        ...
        if worker not in self.rt:
            print "&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&"
            self.rt[worker] = self.factory.fromWorkerToCSServers[1]
        self.sendLine(data)
...
class TransportWorkerFactory(protocol.ClientFactory):
    def __init__(self, address, homedir, fromWorkerToCSClient, fromCSClientToWorker, fromWorkerToCSServers, fromCSServersToWorker):
